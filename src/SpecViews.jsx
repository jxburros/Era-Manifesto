...
export const CombinedTimelineView = () => {
  ...
  const timelineItems = useMemo(() => {
    ...
    // Pre-index releases by their attached items for O(1) lookups instead of O(n) filtering
    const releasesByAttachedSong = new Map();
    const releasesByAttachedVersion = new Map();
    const releasesByAttachedVideo = new Map();
    const releasesByAttachedStandaloneVideo = new Map();

    (data.releases || []).forEach(release => {
      // Index by attached songs
      (release.attachedSongIds || []).forEach(songId => {
        if (!releasesByAttachedSong.has(songId)) releasesByAttachedSong.set(songId, []);
        releasesByAttachedSong.get(songId).push(release.id);
      });
      // Index by attached versions (songId-versionId pair)
      (release.attachedVersions || []).forEach(v => {
        const key = `${v.songId}-${v.versionId}`;
        if (!releasesByAttachedVersion.has(key)) releasesByAttachedVersion.set(key, []);
        releasesByAttachedVersion.get(key).push(release.id);
      });
      // Index by attached videos
      (release.attachedVideoIds || []).forEach(videoId => {
        if (!releasesByAttachedVideo.has(videoId)) releasesByAttachedVideo.set(videoId, []);
        releasesByAttachedVideo.get(videoId).push(release.id);
      });
      // Index by standalone videos
      (release.attachedStandaloneVideoIds || []).forEach(videoId => {
        if (!releasesByAttachedStandaloneVideo.has(videoId)) releasesByAttachedStandaloneVideo.set(videoId, []);
        releasesByAttachedStandaloneVideo.get(videoId).push(release.id);
      });
    });
